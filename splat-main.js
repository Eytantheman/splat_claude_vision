...
async function main() {
    ...
    console.log(req);
    if (req.status != 200)
        throw new Error(req.status + " Unable to load " + plyUrl);
    ...
    worker.onmessage = (e) => {
        if (e.data.buffer) {
            ...
            vertexCount = e.data.vertexCount;
            if (window._splat && window._splat.onDepthUpdate) {
                const cb = window._splat.onDepthUpdate;
    ...
    window.addEventListener("keydown", (e) => {
        if (window._splat?.controlsEnabled === false) {
    ...
    window.addEventListener(
        "wheel",
        (e) => {
            if (window._splat?.controlsEnabled === false) { e.preventDefault(); return; }
            carousel = false;
            e.preventDefault();
            const lineHeight = 10;
            const scale =
    ...
    canvas.addEventListener("mousedown", (e) => {
        if (window._splat?.controlsEnabled === false) return;
        carousel = false;
        e.preventDefault();
        startX = e.clientX;
        startY = e.clientY;
    ...
    const frame = (now) => {
        ...
        isJumping = activeKeys.includes("Space");
    ...
    // Expose programmatic camera control for the annotation tool
    window._splat = {
        getViewMatrix: () => [...viewMatrix],
        setViewMatrix: (m) => { viewMatrix = [...m]; carousel = false; },
        ...
        utils: { invert4, rotate4, translate4, multiply4 },
    };
